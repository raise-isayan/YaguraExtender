 ...
 */
public class FixDigestAuthenticator implements CachingAuthenticator {
    ...
    public FixDigestAuthenticator(Credentials credentials) {
        this.credentials = credentials;
        this.credentialsCharset = StandardCharsets.US_ASCII;
    }

    public FixDigestAuthenticator(Credentials credentials, Charset credentialsCharset) {
        this.credentials = credentials;
    ...
    private Request authenticateWithState(Route route, Request request, Map<String, String> parameters)
        ...
        if (route == null || !route.requiresTunnel()) {
            ...
            if (isProxy()) {
                ...
                /* Proxy の先がDigest認証のケース */
                parameters.put("methodname", request.method());                                
                parameters.put("uri", this.requestPath(request.url()));
            }
        }
