...
public class JWTTokenTest {
    ...
    private final static Logger logger = Logger.getLogger(JWTTokenTest.class.getName());
    ...
    public void testJWTHeader() {
        {
            String token = JWTToken.jwtHeader(JWTToken.Algorithm.NONE);
            assertEquals("{\"alg\":\"NONE\",\"typ\":\"JWT\"}", token);
        }
        {
            String token = JWTToken.jwtHeader(JWTToken.Algorithm.HS256);
            assertEquals("{\"alg\":\"HS256\",\"typ\":\"JWT\"}", token);
        }
    }
    ...
    public void testSignatureEqual_args4() {
        ...
        String token256 = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJtYWluIjoieHh4eHh4Iiwic3ViIjoi44OG44K544OIIn0";
        String token384 = "eyJhbGciOiJIUzM4NCIsInR5cCI6IkpXVCJ9.eyJtYWluIjoieHh4eHh4Iiwic3ViIjoi44OG44K544OIIn0";
        String token512 = "eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9.eyJtYWluIjoieHh4eHh4Iiwic3ViIjoi44OG44K544OIIn0";
        {
            String encodeSig = "IjbkfaSdmROAC0MeW40lJo4s_KoX0VgF0vogsXygNNc";
            boolean result = JWTToken.signatureEqual(JWTToken.Algorithm.HS256, token256, JsonToken.decodeBase64UrlSafeByte(encodeSig), "secret");
            assertEquals(expResult, result);
        }
        {
            String encodeSig = "H96erEFQSxsDQmwLwZhZDJCd-sXyfTkDmDGKBPYmAG4";
            boolean result = JWTToken.signatureEqual(JWTToken.Algorithm.HS256, token256, JsonToken.decodeBase64UrlSafeByte(encodeSig), "test");
            assertEquals(expResult, result);
        }
        {
            String encodeSig = "Chui3Q6dauCAgh41YUveD9-S6XR29d5udoNzT7yQkPH6t4lRB-3Ue9ovzGysDSOz";
            boolean result = JWTToken.signatureEqual(JWTToken.Algorithm.HS384, token384, JsonToken.decodeBase64UrlSafeByte(encodeSig), "test");
            assertEquals(expResult, result);
        }
        {
            String encodeSig = "sx5HdJzL5IrFtZPbFnptVUvZ28qTy56vXamIy9hgB0cjc6-Zr2D5s85HSCtmQKWROseztDes9VycyPYYFUYbHA";
            boolean result = JWTToken.signatureEqual(JWTToken.Algorithm.HS512, token512, JsonToken.decodeBase64UrlSafeByte(encodeSig), "test");
            assertEquals(expResult, result);
        }
    ...
    public void testSignatureEqual_args2() {
        ...
        {
            JWTToken token = jwtinstance.parseToken(JWT_TOKEN01, true);
            boolean result = token.signatureEqual("secret");
            assertEquals(expResult, result);
        }
        {
            JWTToken token = jwtinstance.parseToken(JWT_TOKEN02, true);
            boolean result = token.signatureEqual("brady");
            assertEquals(expResult, result);
        }
        {
            JWTToken token = jwtinstance.parseToken(JWT_TOKEN03, true);
    ...
    public void testSignatureSign() {
        ...
        try {
            {
                byte[] sign = JWTToken.sign(JWTToken.Algorithm.HS256, payload, "secret");;
                String result = JsonToken.encodeBase64UrlSafe(sign);
                assertEquals(expResult_HS256, result);
            }
            {
                byte[] sign = JWTToken.sign(JWTToken.Algorithm.HS384, payload, "secret");
                String result = JsonToken.encodeBase64UrlSafe(sign);
                assertEquals(expResult_HS384, result);
            }
            {
                byte[] sign = JWTToken.sign(JWTToken.Algorithm.HS512, payload, "secret");
                String result = JsonToken.encodeBase64UrlSafe(sign);
...
//            {
//                byte [] privateKey = Util.readAllBytes(JWTUtilTest.class.getResourceAsStream("/resources/private-key.pem"));
//                byte [] sign = JWTToken.sign(JWTToken.Algorithm.RS384, header_RS384 + token, Util.getRawStr(privateKey));
//                String result = JWTToken.encodeBase64UrlSafe(sign);
//                assertEquals(expResult_RS384, result);
//            }
//            {
//                byte [] privateKey = Util.readAllBytes(JWTUtilTest.class.getResourceAsStream("/resources/private-key.pem"));
//                byte [] sign = JWTToken.sign(JWTToken.Algorithm.RS512, header_RS512 + token, Util.getRawStr(privateKey));
//                String result = JWTToken.encodeBase64UrlSafe(sign);
...
//        }
    ...
    public void testParseJWTToken() {
        ...
        {
            String value = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.e30.Et9HFtf9R3GEMA0IICOfFMVXY7kkTX1wr4qCyhIf58U";
            JWTToken expResult = null; // payload = "{}"
            JWTToken result = jwtinstance.parseToken(value, true);
            assertNotEquals(expResult, result);
        }
        {
            String value = "xeyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ5mhBHqs5_DTLdINd9p5m7ZJ6XD0Xc55kIaCRY5r6HRA";
            JWTToken expResult = null;
            JWTToken result = jwtinstance.parseToken(value, false);
            assertEquals(expResult, result);
        }
        {
            String value = "eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.";
...
//        }
    ...
    public void testGenerateHSToken() {
        {
            ...
            try {
                ...
                {
                    JWTToken token = jwtinstance.parseToken(test, true);
                    byte[] sign = JWTToken.sign(JWTToken.Algorithm.HS256, token.getPayload(), publicKey);
                    String signature = JsonToken.encodeBase64UrlSafe(sign);
                    String result = JsonToken.encodeBase64UrlSafe(JWTToken.jwtHeader(JWTToken.Algorithm.HS256)) + "." + token.getPayload() + "." + signature;
                    //System.out.println("expected:" + expected);
                    System.out.println("result:" + result);
                    assertEquals(expected, result);
                }
                {
                    String[] tokens = JWTToken.generatePublicToHashToken(test, publicKey);
    ...
    public void testJWTTokenNothing() {
        ...
        {
            boolean expResult = false;
            boolean result = JWTToken.containsTokenFormat(REQ_MESSAGE_URLENCODE_TOKEN00);
            assertEquals(expResult, result);
        }
        {
            CaptureItem[] token = JWTToken.findToken(REQ_MESSAGE_URLENCODE_TOKEN00);
    ...
    public void testJWTToken() {
        ...
        {
            boolean expResult = true;
            boolean result = JWTToken.containsTokenFormat(REQ_MESSAGE_URLENCODE_TOKEN01);
            assertEquals(expResult, result);
        }
        {
            ...
            CaptureItem[] token = JWTToken.findToken(REQ_MESSAGE_URLENCODE_TOKEN01);
    ...
    public void testJWTTokenRequest() {
        System.out.println("testJWTTokenRequest");
        {
            JWTToken jwtInstance = new JWTToken();
            String expResult = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJtYWluIjoiYWJjZGVmIiwic3ViIjoiaG9nZSIsInllYXIiOjIwMjB9.bfk79BN28BVvW6lRnITaEULZ7URDBcem4jalLOW5diM";
            JWTToken result = jwtInstance.parseToken(REQ_MESSAGE_URLENCODE_TOKEN01, false);
            assertEquals(JWTToken.Algorithm.HS256, result.getAlgorithm());
