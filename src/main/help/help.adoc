...
Run the selected extension.
  ...
  Paste Root URL into include Target scope (multi-line)::
    Add a multi-line Root URL set on the clipboard to the Target Scope Include in scope.
...
Intarcept Toggle Bottn::

It has the same functionality as the Intercept feature in the Proxy tab and works in conjunction with Burp core behavior.

Intarcept icon::

You can configure the Request interception rules, Response interception rules and WebSocket interception rules in the proxy settings.
...
Switch to Intarcept tab::
Select the [Proxy] -> [Intercept] tab.

Switch to Intarcept tab AUTO button::
Automatically selects the [Proxy] -> [Intercept] tab.

Open Browser::

Launch the browser with the selected profile.

Manage global settings icon::

You can load and save User and Project settings.
...
[JavaScript Comment] check box::
    Display JavaScript Comment tab in message tab.
...
[Floatable] check box::
  Enable floating Burp suite Toolbar. 

[Default AUTO button state] combobox::
  OFF ::  Hide
  ON :: Show
...
If checked for sending to server, 
Customize the request parameters used to send SendTo.

image:images/custom_sendto_server_sendtoparameter.png[SendToItem Edit SendTo Parameter Setting Dialog]
...
===== SendTo Args Configuration Dialog

This can be set if the server is not checked.
Customize the command line arguments used to send SendTo.

image:images/custom_sendto_server_sendtoargs.png[Edit SendToItem SendTo Args Setting Dialog]

override send to args::
  Set command line arguments for SendTo send.

Args::
  Specifies command line arguments.
  Separate them with a new line to make them a single parameter.
  The [Multi Edit] button can be used to change them all at once.
  The argument can be a format parameter.
...
Base64 + URLS::
    Encodes and decodes Base64 and URL format.
...
\b{bbbb}(bin)::
    Encodes and decodes in binary format.
...
KECCAK-224::
    Calculate hash with KECCAK-224

KECCAK-256::
    Calculate hash with KECCAK-256
...
ISAP::
   Calculate hash with ISAP

AsconHash::
   Calculate hash with AsconHash

AsconHashA::
   Calculate hash with AsconHashA

AsconXof::
   Calculate hash with AsconXof

AsconXofA::
   Calculate hash with AsconXofA

ESCH256::
   Calculate hash with ESCH256

ESCH384::
   Calculate hash with ESCH384

PhotonBeetle::
   Calculate hash with PhotonBeetle

Xoodyak::
   Calculate hash with Xoodyak
...
CRC16/ARC::
    Calculate checksum by CRC16/ARC

CRC16/CCIT::
    Calculate checksum by CRC16-CCIT

CRC16-DNP::
    Calculate checksum by CRC16-DNP

CRC16-IBM-SDLC::
    Calculate checksum by CRC16-IBM-SDLC

CRC16-MAXIM::
    Calculate checksum by CRC16-MAXIM

CRC16-MCRF4XX::
    Calculate checksum by CRC16-MCRF4XX

CRC16-MODBUS::
    Calculate checksum by CRC16-MODBUS

CRC16-NRSC-5::
    Calculate checksum by CRC16-NRSC-5

CRC16-USB::
    Calculate checksum by CRC16-USB
...
xxHash32::
    Calculate checksum by xxHash32

Murmur3/32x86::
    Calculate checksum by MurmurHash3 32bit

Murmur3/128x64::
    Calculate checksum by MurmurHash3 128bit
...
* bin2hex
    Converts ASCII binary notation to HEX
* hex2bin
    Convert HEX to ASCII in binary notation
* Full width -> Half width
    Convert full-width characters to half-width characters.
* Half width -> Full width
    Convert half-width characterss to full-width character.
...
The Generater has a Sequence tab and a Random tab and GenerateKeyPair tab.
...
[Output Copy] button::
    Output the generated value to the clipboard.

[Save to file] button::
    Output the generated value to a file.
...
[KeyPair in PEM format]
  Generates an encryption key in PEM format.

[KeyPair in JWK format]
  Generates an encryption key in JWK format.
...
image:images/custom_jtranscoder_ViewState_decoder.png[View State Decoder tab]
...
[To Encode] botton::
  Copy the decoded value to the JWT Encoder tab.
...
image:images/custom_jtranscoder_JWT_encoder.png[JWT token Decoder tab]

[JWT] text area::
  Encode and display the JWT Token.
...
[Algorithm] combobox::
  Select the JWT algorithm.

[Header] text area::
  Input the JWT header to encode.

[Payload] text area::
  Input the JWT payload to encode.

[Secret] text area::
  Input the JWT secret to encode.

==== JWK Converter tab

Convert JWK.

image:images/custom_jtranscoder_JWK_converter.png[JWT token Decoder tab]

[JWK] text area::
  Input the JWK to be converted.
...
image:images/custom_JWT_decoder.png[JWT tab]
