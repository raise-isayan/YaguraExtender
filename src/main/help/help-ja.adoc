...
[Proxy] -> [Intercept]タブを選択状態にします。

Switch to Intarcept tab AUTO ボタン::

自動で[Proxy] -> [Intercept]タブを選択状態にします。
...
[JS Comment] チェックボックス::
    メッセージタブにJavaScript Commentタブを表示するようにします。
...
[Default AUTO button state] コンボボックス::
  OFF ::  非表示
  ON ::  表示
...
KECCAK-224::
    KECCAK-224によるハッシュを計算します。

KECCAK-256::
    KECCAK-256によるハッシュを計算します。
...
CRC16/ARC::
    CRC16/ARCによるチェックサムを計算します。

CRC16/CCIT::
    CRC16-CCITによるチェックサムを計算します。

CRC16-DNP::
    CRC16-DNPによるチェックサムを計算します。

CRC16-IBM-SDLC::
    CRC16-IBM-SDLCによるチェックサムを計算します。

CRC16-MAXIM::
    CRC16-MAXIMによるチェックサムを計算します。

CRC16-MCRF4XX::
    CRC16-MCRF4XXによるチェックサムを計算します。

CRC16-MODBUS::
    CRC16-MODBUSによるチェックサムを計算します。

CRC16-NRSC-5::
    CRC16-NRSC-5によるチェックサムを計算します。

CRC16-USB::
    CRC16-USBによるチェックサムを計算します。
...
* Lower Case
    小文字に変換します｡
* bin2hex
    2進数表記のASCIIをHEXに変換します
* hex2bin
    HEXを2進数表記のASCIIに変換します
* Full width -> Half width
   全角を半角に変換します。
* Half width -> Full width
   半角を全角に変換します。
...
Generater には、SequenceタブとRandomタブ、GenerateKeyPairタブがあります。
...
image:images/custom_jtranscoder_JWT_encoder.png[JWT Token Decoderタブ]
...
[Header] テキストエリア::
 エンコードするJWTのHeaderを入力します。

[Payload] テキストエリア::
 エンコードするJWTのPayloadを入力します。

[Secret]テキストエリア::
 エンコードするJWTのSecretを入力します。

==== JWK Converter タブ

JWKを変換します。

image:images/custom_jtranscoder_JWK_converter.png[JWT Token Decoderタブ]
